"""
文件解析服务：仅解析 PDF。
每个 PDF 生成一个 id（文件夹），id 下存 content.md（文字内容）与图片；图片若无描述则自动生成。
"""
...
# 返回: (id, content_path, error_message, failed_caption_count)
# 成功时 content_path 为 id 文件夹下的 content.md 绝对路径
ParseResult = Tuple[Optional[str], Optional[str], Optional[str], int]


class FileParserService:
    """
    仅解析 PDF。每个文件对应一个 id 文件夹：{storage_root}/{id}/content.md + 图片。
    图片若无文字描述则自动调用视觉模型生成。
    """
    ...
    def parse_file(self, file_path: str, filename: str = None) -> ParseResult:
        """
        仅支持 PDF。解析后生成一个 id 文件夹，内含 content.md 与图片；无描述的图片自动生成描述。

        Returns:
            (id, content_md_path, error_message, failed_caption_count)
            id: 本次解析的文件夹名（即 extract_id）
            content_md_path: content.md 的绝对路径；失败为 None
        """
        path = Path(file_path)
        if not path.is_file():
        ...
        if not self.mineru_token or not self.mineru_token.strip():
            return None, None, "请配置 MINERU_TOKEN（见 文件解析/config.py）", 0
    ...
    def _poll_result(self, batch_id: str, max_wait: int = 600) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        """返回 (zip_url, None, None) 或 (None, None, error)。"""
        headers = {"Content-Type": "application/json", "Authorization": f"Bearer {self.mineru_token}"}
    ...
    def _download_and_save(self, batch_id: str) -> Tuple[Optional[str], Optional[str]]:
        """
        轮询结果、下载 ZIP、解压到 storage_root/{id}/，写出 content.md（相对路径引用图片）。
        返回 (extract_id, error)。
        """
        ...
        # 图片路径改为相对路径（相对 content.md 所在目录），便于 id 文件夹自包含
    ...
    def _replace_image_paths_to_relative(self, content: str, md_rel_path: str) -> str:
        """将 Markdown 内图片路径统一为相对路径（相对 md 所在目录）。"""
    ...
    def _enhance_markdown_with_captions(self, content: str, extract_id: str) -> Tuple[str, int]:
        """为无 alt 的图片生成描述；图片路径为相对路径，解析为 id_dir/rel。"""
        pattern = r"!\[(.*?)\]\(([^\)]+)\)"
    ...
    @staticmethod
    def extract_header_footer_from_layout(extract_id: str, storage_root: Path = None) -> str:
        """从 layout.json 的 discarded_blocks 提取页眉页脚。"""
        from .config import get_storage_root
        root = storage_root or get_storage_root()
        layout_file = Path(root) / extract_id / "layout.json"
...
def extract_header_footer_from_layout(extract_id: str, storage_root: Path = None) -> str:
    return FileParserService.extract_header_footer_from_layout(extract_id, storage_root)
