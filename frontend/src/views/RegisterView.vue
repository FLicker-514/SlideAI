...
async function onSubmit() {
  errMsg.value = ''
  if (!username.value.trim() || !email.value.trim() || !password.value || !code.value) {
  ...
  try {
    ...
    if (res.data.code === 200) {
      ...
      errMsg.value = res.data.message || '注册失败，请检查验证码是否有效（10 分钟内有效）'
    }
  } catch (e: unknown) {
    const ax = e as { response?: { data?: { message?: string } } }
...
<template>
  <div class="page">
    <div class="card">
      ...
      <p class="card-desc">填写信息并完成邮箱验证</p>
      <form class="form" @submit.prevent="onSubmit">
        <label class="label">用户名</label>
        <input v-model="username" type="text" placeholder="请输入用户名" required class="input" />
        <label class="label">邮箱</label>
        <input v-model="email" type="email" placeholder="用于接收验证码" required class="input" />
        <div class="field-row">
        ...
        <input v-model="password" type="password" placeholder="请设置密码" required class="input" />
